...
| **Panel layout** | Registers / Memory / Peripherals / Disassembly are independent (not exclusive tabs): show any combination; ≤2 docked panels stack vertically, >2 form a **2×2 grid** (dock expands to half the window width) with draggable row/column splitters; detach any panel into a draggable, resizable floating window. Layout persists to `localStorage`. Each of the three data panels (Registers / Memory / Disassembly) also has a **draggable column splitter** (a "◂ Columns ▸" knob) that resizes its fixed data columns (e.g. `#`, `Name`, `Addr`, `Machine code`) — the elastic columns (Value / Native / Source / ASCII) automatically absorb the leftover width, so the fixed-width data never scales or wraps even when the dock widens. Column widths persist per panel. The Peripherals panel is untouched. |
...
**v23.8** — Intra-panel column splitters & no-wrap data columns:
- **Draggable column splitters inside the data panels**: Registers, Memory, and Disassembly each have a **`◂ Columns ▸`** knob bar (`.panel-colsplit`) at the top of their content. Dragging it (or clicking **Reset** / double-clicking) resizes the panel's *fixed* data columns (`#`, `Name`, `Addr`, `Machine code`, hex block) while the *elastic* columns (`Value (Hex/Dec)`, `ASCII`, `Native instruction`, `Original source`) automatically absorb the leftover width. Per-panel widths persist under `rvsim.panelCols.<name>`. The Peripherals panel is untouched.
- **No unnecessary data wrapping when widening the dock**: the fixed data columns now use `white-space: nowrap` so their contents always fit on one line. The Disassembly table uses an auto-layout `colgroup` with `col-fixed` (Addr + Machine code) and `col-flex` (Native + Source); the Memory rows are flex blocks (Addr · hex · ASCII) so the hex data never wraps. Previously widening the dock scaled every column proportionally, wrapping short addresses/hex values.
- New regression assertions in `riscv_simulator_tests/test_panel_grid.js` (sections [16]–[18]).
