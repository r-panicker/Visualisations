...
| **Panel layout** | Registers / Memory / Peripherals / Disassembly are independent (not exclusive tabs): show any combination; ≤2 docked panels stack vertically, >2 form a **2×2 grid** (dock expands to half the window width) with draggable row/column splitters; detach any panel into a draggable, resizable floating window. Layout persists to `localStorage`. The three data panels (Registers / Memory / Disassembly) share one header design with **always-visible draggable column separators**: content-sized columns (`#`, `Name`, `Addr`, `Content (Hex)`) never stretch as the dock widens, the text columns share the surplus between them, and a too-narrow panel scrolls rather than crushing a column. All three freeze their header row while the data scrolls. Dragging is spreadsheet-style (columns to the right shift along); double-click a separator to reset. Widths persist per panel. The Peripherals panel is untouched. |
...
**v23.8** — Intra-panel column-resize separators, unified data-panel headers & no-stretch columns:
- **One header design and one separator across all three data panels**: Registers, Memory (a new `.mem-col-header` label bar above the hex dump) and Disassembly now share the same small-uppercase header row and an **always-visible hairline separator** (`.col-resizer`) at every column boundary but the last — it thickens to mauve under the pointer and has an 11px hit area, so boundaries are discoverable at a glance instead of having to be hunted for. All three headers are **frozen** (`position: sticky`) so they stay put while the rows scroll beneath them, and Memory's columns are labelled **Addr / Content (Hex) / Content (ASCII)**.
- **Columns never stretch just because the panel got wider**: `PANEL_COLS`/`applyPanelColLayout()` size every column and write the result into each table's `<colgroup>` (and Memory's `--mem-*-w` custom properties). `#`, `Name`, `Addr`, `Machine code` and `Content (Hex)` hold their content-sized width; the surplus is **shared between** the text columns (`Value (Hex)`+`Value (Dec)`, `Native instruction`+`Original source`, `Content (ASCII)`) rather than dumped on whichever column happens to be last.
- **Narrow panels scroll instead of crushing a column**: below the sum of the column minimums the widths hold and the panel scrolls horizontally. The text columns take up the slack first and wrap at word boundaries (no more `break-all` chopping mnemonics mid-token).
- **Spreadsheet-style dragging**: the first movement pins every column at its rendered width, then only the dragged column changes — so everything to its right keeps its width and simply shifts along. A plain click pins nothing; **double-click** any separator to unpin the panel and return to the automatic layout. A `ResizeObserver` re-runs the layout on panel resize.
- **Fixes from the first attempt**: `width: 1%` on the elastic columns had *collapsed the last two columns while ballooning the first two* (in a fixed-layout table where every column has a specified width, surplus is shared in proportion to those widths). Memory's `Content (Hex)` | `Content (ASCII)` boundary is now clamped to the hex data's measured natural width, so the separator can no longer be dragged back over the bytes.
- Widths persist per panel in `localStorage` under `rvsim.panelColW.<panel>`. The **Peripherals** panel is untouched.
- New regression assertions in `riscv_simulator_tests/test_panel_grid.js` (sections [16]–[18]), including a forced-width case proving that widening a panel feeds the text columns and never stretches `Addr`/`Machine code`.
