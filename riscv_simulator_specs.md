...
- **Read-Only Code Protection**: The **Code (.text)** segment in the Memory View is strictly read-only to prevent accidental program corruption during inspection, while **Data** and **Stack** retain full interactive byte- and word-level editing; **MMIO** is editable per-register — read-only registers (DIP, PB, ...) render the same as Code (§7.3).
