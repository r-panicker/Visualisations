...
| File | What it is |
|---|---|
| [`riscv_simulator.html`](riscv_simulator.html) | The simulator. Single file, no build step; everything but C compilation works offline. Runs programs on either of **two execution engines** — the built-in JavaScript functional model, or **your own Verilog processor** compiled and simulated in the browser. |
...
Compiler Explorer (Godbolt) REST API, which is the one part that needs the network;
everything else — assembling, simulating, the HDL engine — runs in the browser.
