...
- **Intra-panel column splitters**: Each of the three data panels — **Registers**, **Memory**, and **Disassembly** — has a small **`◂ Columns ▸`** knob bar (`.panel-colsplit`) at the top of its content. Dragging it (or using the **Reset** button / double-click) resizes the panel's *fixed* data columns (`#`, `Name`, `Addr`, `Machine code`, hex block) while the *elastic* columns (`Value (Hex/Dec)`, `ASCII`, `Native instruction`, `Original source`) automatically absorb the leftover width — so the short fixed-width data always stays on one line and never scales/wraps as the panel or dock widens. The widths persist per panel under `rvsim.panelCols.<name>`. The **Peripherals** panel is deliberately left untouched.
...
- **Fixed data columns / elastic text columns**: The **Addr** and **Machine code** columns are *fixed-width* (`col-fixed`) — they stay at their natural size so the hex address/machine-code word always fits on one line, and they never scale up when the dock widens. The **Native instruction** and **Original source** columns are *elastic* (`col-flex`) and share the leftover panel width, so the fixed-width data never wraps unnecessarily.
...
|---------|----------------------------------------------|
| **v23.6 (Intra-panel Column Splitters & No-Wrap Data Columns)** | Added **draggable column splitters** inside the **Registers**, **Memory**, and **Disassembly** panels — a **`◂ Columns ▸`** knob (`.panel-colsplit`) with a **Reset** button (double-click also resets) that resizes the panel's *fixed* data columns (`#`, `Name`, `Addr`, `Machine code`, hex block) while the *elastic* columns (`Value (Hex/Dec)`, `ASCII`, `Native instruction`, `Original source`) automatically absorb the leftover width. Fixed data columns now keep `white-space: nowrap` so their contents always fit on one line and never scale/wrap as the dock widens (previously widening the dock expanded every column proportionally, wrapping short addresses/hex unnecessarily). The Disassembly table switched from `table-layout: fixed` to an auto-layout `colgroup` with `col-fixed` (Addr + Machine code) and `col-flex` (Native + Source); the Memory rows are now flex blocks (Addr · hex · ASCII) so the hex data never wraps and the ASCII column flexes. Per-panel column widths persist under `rvsim.panelCols.<name>` in `localStorage`. The **Peripherals** panel is untouched. |
